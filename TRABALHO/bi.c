...
void Teclado (unsigned char key, int x, int y)  
{
    ...
    if(gameover == 0 ){
		...
		switch(key)
		{
			...
			case 'R':   //VERMELHO    
				intCores = 0;                //Set cor 
				jogar(0);                    //ALimenta arrayDeJogadas
				montaQuebracabeca();         // Cria jogada randomica
				contGlogal = 0;              //Zera contador Global
				exibirMenssagemJogar = 0;    //Retira a menssagem ENJOY
				break;
			case 'r':   //VERMELHO    
				intCores = 0;
				jogar(0);
